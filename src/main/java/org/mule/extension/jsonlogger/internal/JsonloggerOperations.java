...
public class JsonloggerOperations {
    ...
    public void logger(@ParameterGroup(name = "Logger") @Expression(value = NOT_SUPPORTED) LoggerProcessor loggerProcessor,
        ...
        String serverName = loggerProcessor.getServerName().toString();
        
        String logType = loggerProcessor.getType().toString();
        ...
        if (isLogEnabled(loggerProcessor.getPriority().toString())) {
             ...
             **/
            // 1. Location Info: Logger location within Mule application
            ...
            // 2. Remote Address : Remote Address of API Consumer
            mergedLogger.put("server_name", serverName);
            
            // 3. Log Type : Log Type debug , info etc
            mergedLogger.put("type", logType);
            
            // 4. Timestamp: Add formatted timestamp entry to the logger
            mergedLogger.put("timestamp", getFormattedTimestamp(loggerTimestamp));
            
            // 5. Extra fields: String based fields
            ...
            // 6. Extra fields: JSONNode based fields
           ...
           // 7. Remove Priority field from JSON Payload
            mergedLogger.remove("priority");
    ...
    public void loggerScope(@DisplayName("Module configuration") @Example("JSON_Logger_Config") @Summary("Indicate which Global config should be associated with this Scope.") String configurationRef,
        ...
        if (isLogEnabled(priority.toString())) {
            ...
            loggerProcessor.put("dist", configs.getConfig(configurationRef).getGlobalSettings().getDist());
            loggerProcessor.put("release", configs.getConfig(configurationRef).getGlobalSettings().getRelease());
            loggerProcessor.put("environment", configs.getConfig(configurationRef).getGlobalSettings().getEnvironment());
            loggerProcessor.put("platform", configs.getConfig(configurationRef).getGlobalSettings().getPlatform());
